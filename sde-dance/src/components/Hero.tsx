...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    tl
      ...
      // 4. Headline — each word rises from below like a stage reveal
      .fromTo(
        wordInners,
        { y: "105%", opacity: 0 },
        { y: "0%", opacity: 1, duration: 0.65, stagger: 0.055 },
        "-=0.2"
      )
  ...
  return (
    ...
    >
      ...
      {/* TODO-CLIENTE: trocar por <video> de espetáculo em loop mudo */}
      <div className="absolute inset-0 z-0" aria-hidden="true" style={{
        background: [
          "radial-gradient(ellipse 85% 65% at 50% 28%, rgba(110,16,35,0.24) 0%, transparent 65%)",
      ...
      {/* Film grain */}
      <div className="absolute inset-0 z-0 opacity-[0.25] mix-blend-overlay pointer-events-none" aria-hidden="true" style={{
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 z-0 pointer-events-none" aria-hidden="true" style={{
        width: "1px",
        height: "50%",
        background: "linear-gradient(to bottom, rgba(231,182,92,0.4) 0%, transparent 100%)",
        boxShadow: "0 0 80px 40px rgba(231,182,92,0.07)",
      }} />

      {/* Giant "10" — background ghost typography (Paris Opera / NDT style) */}
      <div ref={stageNumRef} className="absolute select-none pointer-events-none z-0" aria-hidden="true" style={{
        opacity: 0,
        right: "-1vw",
        top: "50%",
        ...
        lineHeight: 1,
        color: "transparent",
        WebkitTextStroke: "1px rgba(110,16,35,0.2)",
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 lg:px-10 w-full pt-28 pb-24">
        <div className="max-w-4xl">
          ...
          {/* Sub */}
          <p ref={subRef} className="text-lg md:text-xl max-w-xl leading-relaxed mb-12" style={{ color: "var(--color-ash)", opacity: 0 }}>
            {site.hero.sub}
          </p>
      ...
      <div className="absolute bottom-8 left-6 lg:left-10 flex items-center gap-3" aria-hidden="true">
        ...
        <span className="text-[0.6rem] tracking-[0.25em] uppercase" style={{ color: "var(--color-ash)", fontFamily: "var(--font-mono)" }}>
          scroll
        </span>
      </div>
